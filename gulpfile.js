...
gulp.task('font-face:build', function () {
    return gulp.src('src/fonts/*.{otf,ttf}')
        .pipe(gulp.dest('build/fonts'));
});

gulp.task('build', gulp.parallel('html:build', 'js:build', 'style:build', 'img:build', 'font-face:build'));
...
gulp.task('watch', function () {
    ...
    watch('src/fonts/*.{woff,woff2}', gulp.series('font-face:build'));
});
